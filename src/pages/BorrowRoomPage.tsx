...
const BorrowRoomPage = () => {
  ...
  const fetchUserStats = () => {
    axios.get(`${API_BASE_URL}/user/room-borrow-request/me`, {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    axios.post(`${API_BASE_URL}/user/room-borrow-request`, formData, {
      headers: { "Authorization": `Bearer ${localStorage.getItem("jwt")}` },
    }).then((response) => {
    ...
    <div className="flex justify-center items-start h-screen bg-gray-100 p-8 space-x-8">
      ...
      <Paper className="bg-white p-8 rounded shadow-md w-full max-w-md space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between">
            <span className="font-medium">Tổng Số Phiếu Mượn Phòng:</span>
            <span>{userStats?.totalBorrowRequest}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-medium">Số Phiếu Mượn Phòng Quá Hạn:</span>
      ...
      <Dialog open={successDialogOpen} onClose={() => setSuccessDialogOpen(false)}>
        ...
        <DialogContent>Phiếu mượn phòng đã được tạo thành công!</DialogContent>
        <DialogActions>
